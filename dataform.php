...
<body>
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            <a href="dataform2.php"><button  role="button" class="btn-btn btn-md btn-success" value="View More">Next</button></a>
                        </ol>
                    </div>
                </div>
            </div>    
    ...
    <table class="table table-stripped" style="padding-left: -4px;color: #558B2F">
      <thead style="color: green">
        ...
        <th scope="col">Hour</th><th>Location</th><th>Location_no</th>
    ...
    $sql = "SELECT Hour,day,location,location_no,iR,iX,h,vv,n,dd,ff,I1,sn1,t1,t2,t3,I2,sn2,td1,td2,td3,I4,p1,p2,p3,p4,I6,r1,r2,r3,tr,I7,wa,wb,w1,w2,I8,nh,cl,cm,ch,I9,h1,h2,h3,h4,I3,t3i,t3ii,slash1,slash2,I21,sn3,tntx1,tntx2,tntx3,I5,I89,p241,p242,p243,Ieight,ns,c,hs1,hs2,Inine,sp1,sp2,sp3,sp4,atherm,levelpress,baroread,wetbulb,rhumidity,vappressure FROM parameter1";
    $result = $conn-> query($sql);

    if($result-> num_rows > 0) {
        while ($row = $result-> fetch_assoc()){         

        echo "<tr><td>".$row["Hour"]."</td><td>".$row["location"]."</td><td>".$row['location_no']."</td><td>".($row['iR']+$row['iX']+$row['h']+$row['vv'])."</td><td>".($row['n']+$row['dd']+$row['ff'])."</td><td>".($row['I1']+$row['sn1']+$row['t1']+$row['t2']+$row['t3'])."</td><td>".($row['I2']+$row['sn2']+$row['td1']+$row['td2']+$row['td3'])."</td><td>".($row['I4']+$row['p1']+$row['p2']+$row['p3']+$row['p4'])."</td><td>".($row['I6']+$row['r1']+$row['r2']+$row['r3']+$row['tr'])."</td><td>".($row['I7']+$row['wa']+$row['wb']+$row['w1']+$row['w2'])."</td><td>".($row['I8']+$row['nh']+$row['cl']+$row['cm']+$row['ch'])."</td><td>".($row['I9']+$row['h1']+$row['h2']+$row['h3']+$row['h4'])."</td><td>".($row['I3']+$row['sn1']+$row['t1']+$row['t2']+$row['t3'])."</td><td>".($row['t3i']+$row['t3ii']+$row['slash1']+$row['slash2'])."</td>";  
